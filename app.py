...
display_stages = ["Group Stage", "Round of 32", "Round of 16", "Quarter-finals", "Semi-finals", "Final"]

# Mapeamento para Bandeiras (ISO 2-letter codes for FlagCDN)
iso_map = {
...
stage_idx = st.sidebar.select_slider("Select Stage:", options=list(range(len(display_stages))), format_func=lambda x: display_stages[x])

st.sidebar.markdown(f"### Active Teams ({display_stages[stage_idx]})")
...
# 3. Mapa
data = [{"Country_Code": "AAA", "Country": "Active", "Status": "Active"}, {"Country_Code": "BBB", "Country": "Eliminated", "Status": "Eliminated"}, {"Country_Code": "CCC", "Country": "Not Qualified", "Status": "Not Qualified"}]
for code, elim_index in teams_elimination.items():
    data.append({"Country_Code": code, "Country": country_names.get(code, code), "Status": "Eliminated" if stage_idx > elim_index else "Active"})
